...
int  createSignal_complex_1R2star_f(const gsl_vector * x, void *data, gsl_vector * f);
int  createSignal_complex_1R2star_df(const gsl_vector * x, void *data, gsl_matrix * J);
int  createSignal_complex_1R2star_fdf(const gsl_vector * x, void *data,gsl_vector * f, gsl_matrix * J);
                  ...
                  int nrhs, const mxArray *prhs[] )
{
  ...
  const mwSize *imDims;
  double *imsr, *imsi, *te, fieldStrength;
...
/*   mexPrintf("%s%f%s%f%s%f\n", "initwi1: ", initWi[0], ", te1: ", te[0], ", te2 :", te[1]);  */
  ...
  f.f = &createSignal_complex_1R2star_f;
  f.df = &createSignal_complex_1R2star_df;
  f.fdf = &createSignal_complex_1R2star_fdf;
  f.n = n;
  f.p = p;
  f.params = &d;
...
/*         mexPrintf("%s%d%s%f%s%f%s%f%s%f\n", "nf: " , nf, "swr: ", swr[kt],", swi: ", swi[kt]," , sfr: ", sfr[kt],", sfi: ", sfi[kt]); */
  ...
  for(kx=0;kx<nx;kx++) {
	...
	cursi[kt] = imsi[kx + ky*nx + kt*nx*ny];
      ...
      initEst[0] = initWr[kx + ky*nx];
      initEst[1] = initWi[kx + ky*nx];
      initEst[2] = initFr[kx + ky*nx];
      initEst[3] = initFi[kx + ky*nx];
      initEst[4] = initR2[kx + ky*nx];
      initEst[5] = initFieldmap[kx + ky*nx];
      
/*       mexPrintf("%s%f%s%d%s%d\n", "initwi1: ", initWi[0], ", kx : ", kx, ", ky :", ky);  */
...
/* 	  print_state (iter, s); */
      ...
      outWr[kx + ky*nx] = gsl_vector_get (s->x, 0); 
      outWi[kx + ky*nx] = gsl_vector_get (s->x, 1);
      outFr[kx + ky*nx] = gsl_vector_get (s->x, 2); 
      outFi[kx + ky*nx] = gsl_vector_get (s->x, 3);
      outR2[kx + ky*nx] = gsl_vector_get (s->x, 4); 
      outFieldmap[kx + ky*nx] = gsl_vector_get (s->x, 5); 
...
int  createSignal_complex_1R2star_f(const gsl_vector * x, void *data, gsl_vector * f) {
  ...
  double Wr = gsl_vector_get (x, 0);
  double Wi = gsl_vector_get (x, 1);
  double Fr = gsl_vector_get (x, 2);
  double Fi = gsl_vector_get (x, 3);
  double r2 = gsl_vector_get (x, 4);
  double fieldmap = gsl_vector_get (x, 5);
...
/*   } */
  ...
  for(kt=0;kt<nte;kt++) {
    ...
    shatr = cos(2*PI*fieldmap*te[kt])*exp(-te[kt]*r2)*(Wr*swr[kt] + Fr*sfr[kt] - Wi*swi[kt] - Fi*sfi[kt]) - sin(2*PI*fieldmap*te[kt])*exp(-te[kt]*r2)*(Wr*swi[kt] + Fr*sfi[kt] + Wi*swr[kt] + Fi*sfr[kt]);
    shati = sin(2*PI*fieldmap*te[kt])*exp(-te[kt]*r2)*(Wr*swr[kt] + Fr*sfr[kt] - Wi*swi[kt] - Fi*sfi[kt]) + cos(2*PI*fieldmap*te[kt])*exp(-te[kt]*r2)*(Wr*swi[kt] + Fr*sfi[kt] + Wi*swr[kt] + Fi*sfr[kt]);
    
    gsl_vector_set (f, kt,  shatr - cursr[kt]);
    gsl_vector_set (f, kt+nte,  shati - cursi[kt]);
...
int  createSignal_complex_1R2star_df(const gsl_vector * x, void *data, gsl_matrix * J) {
  size_t kt;
  double curJ1,curJ2,curJ3,shatr;
  double curJ4,curJ5,curJ6,shati;
...
int  createSignal_complex_1R2star_fdf(const gsl_vector * x, void *data,gsl_vector * f, gsl_matrix * J) {
  createSignal_complex_1R2star_f (x, data, f);
  createSignal_complex_1R2star_df (x, data, J);
  return GSL_SUCCESS;
}
